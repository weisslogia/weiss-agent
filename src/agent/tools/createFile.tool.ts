...
export class CreateFile extends ITool {
    constructor() {
        super('createFile', 'given a file name and a path create if does not exist a file in that path and return an error if the file exists', { name: z.string(), path: z.string() })
    }
    public async execute(params: { name: string, path: string }) {
        try {
            stdout.write('\n\n')
            logger.info(`Tool: createFile with params: ${JSON.stringify(params)}`)
            const _file = await fs.readFile(path.join(params.path, params.name), { encoding: 'utf-8' })
